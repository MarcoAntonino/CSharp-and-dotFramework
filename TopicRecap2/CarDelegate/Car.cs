        ...
        public void Accelerate(int delta)
        {
            if (carIsDead)
            {
                if (listOfHandlers != null)
                {
                    ...
                    //Infatti ecco perchè non capivo dove gli dicesse di stampare questo messaggio. Glielo dice nel programma dove viene scritto il metodo onCarEngineEvent. RIBADISCO: qui viene passata solo una stringa, quello che ci faremo con questa sitnrga, dipende dal metodo che verrà passato al "setter" RegisterWithCarEngine.
